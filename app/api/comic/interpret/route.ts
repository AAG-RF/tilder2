...
export async function POST(req: NextRequest) {
    ...
    if (!summary || summary.length < 125) {
        return NextResponse.json({ error: "Summary content too short for comic visualization." }, { status: 400 });
    }
            ...
            messages: [
                {
                    role: "system",
                    content:
                        "You are a visual storytelling assistant skilled at turning article summaries into visual panel descriptions for an editorial-style comic strip." + 
